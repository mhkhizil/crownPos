...
import { CreateCustomerDto } from '../../dtos/master-data/master-data-request.dto.js';
...
export class CreateCustomerUseCase {
  ...
  async execute(actorId: string, data: CreateCustomerDto): Promise<CustomerResponseDto> {
    await requirePermission(this.users, actorId, PermissionCode.MANAGE_MASTER_DATA);
    return CustomerResponseDto.fromEntity(await this.repo.createCustomer(data));
  }
}
