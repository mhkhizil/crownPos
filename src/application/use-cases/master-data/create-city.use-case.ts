...
import { CreateCityDto } from '../../dtos/master-data/master-data-request.dto.js';
...
export class CreateCityUseCase {
  ...
  async execute(actorId: string, data: CreateCityDto): Promise<CityResponseDto> {
    await requirePermission(this.users, actorId, PermissionCode.MANAGE_MASTER_DATA);
    return CityResponseDto.fromEntity(await this.repo.createCity(data));
  }
}
