...
import { CreateProductSkuDto } from '../../dtos/master-data/master-data-request.dto.js';
import { ProductSkuResponseDto } from '../../dtos/master-data/master-data-response.dto.js';

@Injectable()
export class CreateProductSkuUseCase {
  constructor(
    @Inject(USER_REPOSITORY) private readonly users: IUserRepository,
    @Inject(MASTER_DATA_REPOSITORY) private readonly repo: IMasterDataRepository,
  ) {}

  async execute(actorId: string, data: CreateProductSkuDto): Promise<ProductSkuResponseDto> {
    await requirePermission(this.users, actorId, PermissionCode.MANAGE_MASTER_DATA);
    return ProductSkuResponseDto.fromEntity(await this.repo.createProductSku(data));
  }
}
