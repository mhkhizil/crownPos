...
import { CreateGateDto } from '../../dtos/master-data/master-data-request.dto.js';
...
export class CreateGateUseCase {
  ...
  async execute(actorId: string, data: CreateGateDto): Promise<GateResponseDto> {
    await requirePermission(this.users, actorId, PermissionCode.MANAGE_MASTER_DATA);
    return GateResponseDto.fromEntity(await this.repo.createGate(data));
  }
}
