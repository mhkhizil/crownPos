...
import { CreateRawMaterialDto } from '../../dtos/master-data/master-data-request.dto.js';
...
export class CreateRawMaterialUseCase {
  ...
  async execute(actorId: string, data: CreateRawMaterialDto): Promise<RawMaterialResponseDto> {
    await requirePermission(this.users, actorId, PermissionCode.MANAGE_MASTER_DATA);
    return RawMaterialResponseDto.fromEntity(await this.repo.createRawMaterial(data));
  }
}
