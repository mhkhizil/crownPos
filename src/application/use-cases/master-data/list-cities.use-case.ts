...
import { CityListQueryDto } from '../../dtos/master-data/master-data-list-update.dto.js';
...
export class ListCitiesUseCase {
  ...
  async execute(
    ...
    const result = await this.repo.listCities({
      page,
      limit,
      search: query.search,
    ...
    return new PaginatedResponseDto(
      result.items.map((e) => CityResponseDto.fromEntity(e)),
      result.total,
      result.page,
      result.limit,
    );
